...
function wpgexporter_get_galleries() {
  ...
  $ENVIRA_TABLE_GALLERY   = 'posts';

  global $wpdb;
  $gallery_table = $wpdb->prefix . $ENVIRA_TABLE_GALLERY;
...
function wpgexporter_get_gallery($id) {
  ...
  $ENVIRA_TABLE_GALLERYMETA   = 'postmeta';

  global $wpdb;
  $gallerymeta_table = $wpdb->prefix . $ENVIRA_TABLE_GALLERYMETA;

  return $wpdb->get_row( $wpdb->prepare( "select meta_value
                          from {$gallerymeta_table}
                          where meta_key = '_eg_in_gallery' and post_id = %d", $id ) );
}

function wpgexporter_get_gallery_image($id) {
  
  $ENVIRA_TABLE_PICTURES   = 'posts';

  global $wpdb;
  $picture_table = $wpdb->prefix . $ENVIRA_TABLE_PICTURES;
...
<?php } else { ?>
  ...
  $archive_file = 'enviragallery-export-' . time();
...
<tr>
  ...
  <th scope="col" class="manage-column" style="font-size: 16px; width: 70%;">Envira Galleries</th>
